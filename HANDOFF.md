...
- **A spent operating window is answered locally, not asked about (2026-09-04).** Past 1 pm, every slot of `Gym ( 6:00 am to 2:00 pm slot )` has started, so `expected` filtered to empty, the skeleton branch was skipped, and what shipped was a spinner resolving into a bare `No open slots / Try a different date.` — a request whose only possible answer was already known. `windowSpent(resource, isoDate, now)` is true when `knownSlotTimes` returns a window and every slot in it has started; `refreshAvailability` returns before the fetch and draws the real state instantly. An unparseable slot time makes it **false** — not knowing is not knowing it is over, and the request is right then. Verified against the clock: spent from exactly 1:00 PM (the last slot's *start*, not the window's 2 pm end, which was never bookable), never for tomorrow's date, never for a resource with no predictable window.
- **`icons.book` was a clock (2026-09-04).** The key was named for the Book Slot tab it was drawn for, but the geometry is a stopwatch — `<circle r=8>` plus hands plus a crown — so Home's nothing-state, Gyan's `Sports slots` chip and every Book Slot empty state asked for a book and rendered a clock. Renamed to `clock` across all six call sites; **the glyph is unchanged and nothing moved on screen**, only the name became truthful. Kept rather than dropped on the spent-window state, where a clock is the apt icon: the whole message is about the day running past the window.
- **The default resource is the first *live* window, not `resources[0]`.** The list is sorted by start time, so `resources[0]` is the earliest window — after 1 pm the one gym option guaranteed to be over. `firstLiveResource()` picks the first whose window is not spent for the selected date (falling back to `resources[0]`), used at mount and on a facility change. This is the whole fix for the common case; everything below is for when the user goes there deliberately.
- **A spent window is annotated, never disabled.** `buildPicker` options take an optional `note`, drawn as its own right-aligned `.fr-picker-option-note` (and `.fr-picker-btn-note` on the collapsed button — a separate element because the label ellipsises, and concatenated the status would be the first thing a long resource name truncated away). The gym reads `Bookable tomorrow` — the copy names what is still possible rather than what was lost, and points at the day nav, which is the actual recovery. (It is accurate: bookings open 24h ahead, so tomorrow's 6 am slot is *already* open — the option is not waiting to unlock, the user just has to pick another day. `Opens tomorrow` would have been false for exactly that reason.) It stays **selectable** for two reasons: the status belongs to the resource *and the date*, so tomorrow's 6 am gym is live and one tap on the day nav away — disabling globally breaks that, and disabling per-date makes the picker's contents change under the user as they step days; and this codebase already committed to "never swallow the tap, say why" with `blockedReason()` on blocked slot tiles. `paintResourceOptions()` therefore reruns on every date step as well as every facility change.
- **Both reachable empty states carry a reason and an exit.** Spent window: `6 AM – 2 PM is bookable from tomorrow`, title only — no body line, because the title already carries the whole message and the window range is right above it in the picker. One action: `Switch to 3 PM – 11 PM` when the same resource *class* has a live sibling window (matched on `cleanResourceName`, so a spent gym never offers a squash court), else `Try tomorrow`. Genuinely empty day: `No open slots` / `Every slot for this day is taken or past.` + `Try tomorrow`. A server message (`Booking date cannot be in the past.`) now renders through `renderEmpty` instead of a bare `.fr-rail-meta` paragraph. All three keep `buildScheduledList` below them, which the old bare-paragraph path dropped.
...
- The send button carries `.fr-spinner-svg`; `inputEl` is never disabled. **New chat**, **Send** and the **chips** are all disabled together while initialising or sending, so a turn can't be orphaned mid-flight.
- **The disabled state keys off `gyanBusy()` — `sending || loadingReady` — not off `gyanState.ready` (2026-09-04).** Using `!ready` meant a *failed* init or a failed turn (whose `catch` sets `ready = false`) left Send and New chat disabled with no code path left to re-run init: the tab was dead until you switched tabs and back. An init that finished and failed is not busy, it is a retry waiting to happen, so the composer unlocks and the next Send starts a fresh `ensureGyanReady()`.
- **Chips carry `btn.disabled` themselves.** They submit via `composerForm.dispatchEvent(new Event('submit'))`, and a programmatic dispatch does not care that `sendBtn.disabled` is set — so a chip tapped during init used to fire a turn the composer was visibly refusing, and any failure in that turn hit the dead end above. The chip is now disabled by the same `isLoading`, plus a `gyanBusy()` guard in the handler. The background `ensureGyanReady()` is also kicked off *before* the first `paintMessages()` (it sets `loadingReady` synchronously) so the opening frame shows the spinner rather than an enabled Send for one tick.
