...
- `master` ships everything described here, and all of it is released: `v1.0.3` carried the pinned resource collator that had been held back as too small to cut a build for.
- The APK on the phone is `v1.0.1`; `v1.0.2` and `v1.0.3` were both built and released with no device attached, so neither is installed or confirmed on-device. The duplicate-slot bug v1.0.2 fixes was never reproducible on this phone in the first place — it was reported on another student's.
...
- Committed, unreleased, installed: Home's Today/Upcoming rework (see Home section) plus a follow-up the same day — cancelled bookings dropped from Home outright rather than dimmed, and autobook watches merged into Today/Upcoming by their own start time instead of always trailing real bookings, on both Home and My Bookings. Built, patched and installed on-device this session; not yet visually confirmed.
